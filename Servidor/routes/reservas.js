...
router.post('/', protect, async (req, res) => {
  const { sala_id, titulo, descripcion, fecha_inicio, fecha_fin, invitados, equipos } = req.body;
  ...
  try {
    ...
    const [traslapes] = await connection.query(
      ...
      [sala_id, fecha_fin, fecha_inicio]
    );
    ...
    const [users] = await connection.query('SELECT refresh_token FROM usuarios WHERE id = ?', [solicitante.id]);
    if (!users.length || !users[0].refresh_token) {
      await connection.rollback();
      return res.status(401).json({ message: 'No se pudo autenticar con Google Calendar.' });
    }
    oauth2Client.setCredentials({ refresh_token: users[0].refresh_token });

    const [salas] = await connection.query('SELECT calendar_id FROM salas WHERE id = ?', [sala_id]);
    const calendarId = salas.length > 0 && salas[0].calendar_id ? salas[0].calendar_id : 'primary';
    ...
    const event = {
      ...
      attendees: invitados ? invitados.map((email) => ({ email })) : [],
      conferenceData: { createRequest: { requestId: `reserva-${Date.now()}`, conferenceSolutionKey: { type: 'hangoutsMeet' } } }
    };
    const createdEvent = await calendar.events.insert({
      calendarId: calendarId,
      resource: event,
      conferenceDataVersion: 1,
      sendUpdates: 'all' // <-- AÑADIR ESTA LÍNEA
    });

    const newReserva = {
      usuario_id: solicitante.id, sala_id, evento_calendar_id: createdEvent.data.id,
      meet_link: createdEvent.data.hangoutLink, titulo, descripcion, fecha_inicio, fecha_fin, status: 'confirmada'
    };
    const [result] = await connection.query('INSERT INTO reservas SET ?', newReserva);
      const newReservaId = result.insertId;

      // 7. Guardar invitados (asegurando que el creador esté incluido)
      const allInvitados = [...new Set([...(invitados || []), solicitante.email])];
      if (allInvitados.length > 0) {
        const invitadoValues = allInvitados.map(email => [newReservaId, email]);
        await connection.query('INSERT INTO reserva_invitados (reserva_id, invitado_email) VALUES ?', [invitadoValues]);
      }

      // 8. Guardar equipo solicitado
      if (equipos && equipos.length > 0) {
        const equipoValues = equipos.map(equipoId => [newReservaId, equipoId, solicitante.id]);
        await connection.query('INSERT INTO reserva_equipos (reserva_id, equipo_id, usuario_id) VALUES ?', [equipoValues]);
      }
...
// PUT /api/reservas/:id - Actualizar una reserva (lógica completa)
router.put('/:id', protect, async (req, res) => {
  ...
  try {
    ...
    if (reserva.usuario_id !== usuarioActual.id) {
      await connection.rollback();
      return res.status(403).json({ message: 'No tienes permiso para editar esta reserva.' });
    }

    // 2. Verificar conflictos de horario si la sala o fechas cambian
    if (sala_id !== reserva.sala_id || fecha_inicio !== reserva.fecha_inicio || fecha_fin !== reserva.fecha_fin) {
        const [traslapes] = await connection.query(
            'SELECT id FROM reservas WHERE sala_id = ? AND status = "confirmada" AND id != ? AND fecha_inicio < ? AND fecha_fin > ?',
            [sala_id, reservaId, fecha_fin, fecha_inicio]
        );
        if (traslapes.length > 0) {
            await connection.rollback();
            return res.status(409).json({ message: 'Conflicto de horario en la nueva sala o período.' });
        }
    }
    ...
    const [salaOriginal] = await connection.query('SELECT calendar_id FROM salas WHERE id = ?', [reserva.sala_id]);
    const calendarIdOriginal = salaOriginal.length > 0 ? salaOriginal[0].calendar_id : 'primary';
    
    // Mover evento si la sala cambió
    let eventoActualizado;
    if (sala_id !== reserva.sala_id) {
      const [salaNueva] = await connection.query('SELECT calendar_id FROM salas WHERE id = ?', [sala_id]);
      const calendarIdNuevo = salaNueva.length > 0 ? salaNueva[0].calendar_id : 'primary';
      
      eventoActualizado = await calendar.events.move({
        calendarId: calendarIdOriginal,
        eventId: reserva.evento_calendar_id,
        destination: calendarIdNuevo,
      });
    }
    ...
    const eventoFinal = await calendar.events.patch({
        calendarId: eventoActualizado ? eventoActualizado.data.organizer.email : calendarIdOriginal,
        eventId: reserva.evento_calendar_id,
    ...
    await connection.query(
      ...
      [sala_id, titulo, descripcion, fecha_inicio, fecha_fin, reservaId]
    );

    // 5. Actualizar invitados
    await connection.query('DELETE FROM reserva_invitados WHERE reserva_id = ?', [reservaId]);
    ...
    await connection.query('DELETE FROM reserva_equipos WHERE reserva_id = ?', [reservaId]);
    if (equipos && equipos.length > 0) {
      const equipoValues = equipos.map(equipoId => [reservaId, equipoId, usuarioActual.id]);
      await connection.query('INSERT INTO reserva_equipos (reserva_id, equipo_id, usuario_id) VALUES ?', [equipoValues]);
    }
...
router.delete('/:id', protect, async (req, res) => {
  ...
  try {
    ...
    if (reserva.usuario_id !== usuarioActual.id && !usuarioActual.roles.includes('admin')) {
      await connection.rollback();
      return res.status(403).json({ message: 'No tienes permiso para cancelar esta reserva.' });
    }

    // 3. Cancelar el evento en Google Calendar
    if (reserva.evento_calendar_id) {
      oauth2Client.setCredentials({ refresh_token: reserva.refresh_token });
      const calendar = google.calendar({ version: 'v3', auth: oauth2Client });
      
      const [salas] = await connection.query('SELECT calendar_id FROM salas WHERE id = ?', [reserva.sala_id]);
      const calendarId = salas.length > 0 && salas[0].calendar_id ? salas[0].calendar_id : 'primary';
